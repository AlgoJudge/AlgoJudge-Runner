...
- [ ] `gh release list -R AlgoJudge/AlgoJudge-Runner` names the tag just pushed.
...
`algojudge.pl` prints **`Runner v0.1.0`** in four places — a card badge and
...
- `AlgoJudge-Website/tests/content.test.mjs:125` **pins the literal** by regex,
  `Runner v0\.1\.0`, and hard-codes the five repository keys. Correcting
